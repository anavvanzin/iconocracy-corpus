...
def test_load_rater2_results_skips_dry_run_rows(tmp_path):
    results_path = tmp_path / "rater2_results.jsonl"
...
def test_make_bootstrap_sample_units_preserves_duplicate_draws():
    paired = {
    ...
    assert [unit_id for unit_id, _ in units] == [
        "ITEM-1__draw0",
        "ITEM-1__draw1",
        "ITEM-3__draw2",
    ]
...
def test_adjust_allocation_to_target_can_reduce_overshoot():
    strata = {
        "fundacional": [{}] * 8,
        "normativo": [{}] * 8,
        "militar": [{}] * 8,
    ...
    assert sorted(adjusted.values()) == [7, 7, 8, 8]


def test_collect_pending_records_excludes_existing_output_ids():
    records = [
    ...
    assert [record["item_id"] for record in pending] == ["A"]


def test_resolve_sample_image_path_uses_exported_sample_metadata(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("tools.scripts.irr_rater2_batch.REPO_ROOT", repo_root)
