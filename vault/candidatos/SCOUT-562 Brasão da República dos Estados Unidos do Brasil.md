---
id: SCOUT-562
tipo: corpus/candidato
status: candidato
titulo: Brasão da República dos Estados Unidos do Brasil
url: https://www.planalto.gov.br/ccivil_03/constituicao/brasao.htm
data_estimada: 1889–
pais: BR
regime: NORMATIVO
confianca: baixo
tags:
- corpus/candidato
- pais/BR
- regime/normativo
- null
related:
- '[[endurecimento]]'
- '[[Feminilidade de Estado]]'
data_scout: 2026-06-23
records_item_id: dcae9d79-deb4-4bcf-af44-2ea43566e52e
motivo_alegorico: Republic
seculo: 19th
---

## Brasão da República dos Estados Unidos do Brasil

### Identificação
**Data**: 1889– | **País**: BR | **Regime**: NORMATIVO
**URL**: [link](https://www.planalto.gov.br/ccivil_03/constituicao/brasao.htm)

**Motivos identificados**: figura feminina alegorica, dispositivo estatal brasileiro

### Análise preliminar de endurecimento
**Regime preliminar**: NORMATIVO

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
BRASIL. Decreto que institui o brasão de armas da República dos Estados Unidos do Brasil. 1889.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `dcae9d79-deb4-4bcf-af44-2ea43566e52e`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
