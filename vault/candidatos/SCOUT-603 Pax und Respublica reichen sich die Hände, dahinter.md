---
id: SCOUT-603
tipo: corpus-candidato
status: candidato
titulo: "Pax und Respublica reichen sich die Hände, dahinter Caritas, Justitia und Prudentia."
acervo: "Herzog August Library"
url: "http://diglib.hab.de?grafik=35-5-geom-2f-2s-00004"
data_estimada: ""
pais: BR
suporte: indeterminado
motivo_alegorico: "Justitia"
regime: INDETERMINADO
confianca: baixo
tags:
  - corpus/candidato
  - pais/BR
  - motivo/justitia
  - #verificar
  - hunt-candidate
related:
  - "[[endurecimento]]"
  - "[[Feminilidade de Estado]]"
hunt_score: 0.5
hunt_source: europeana
data_scout: 2026-07-25
---

## Pax und Respublica reichen sich die Hände, dahinter Caritas, Justitia und Prudentia.

### Identificação
**Criador/Gravador**: Unbekannt  (Formschneider)
**Acervo**: Herzog August Library
**URL**: [link](http://diglib.hab.de?grafik=35-5-geom-2f-2s-00004)
**Data**: indeterminada | **País**: BR | **Suporte**: indeterminado
**Thumbnail**: [imagem](https://api.europeana.eu/thumbnail/v2/url.json?uri=http%3A%2F%2Fdiglib.hab.de%2Fvaria%2Fgrafik%2F35-5-geom-2f-2s-00004%2Fmax%2F000001.jpg&type=IMAGE)
**Direitos**: http://creativecommons.org/licenses/by-sa/3.0/

**Motivos identificados**: Justitia

### Análise preliminar de endurecimento
**Regime**: indeterminado — requer análise visual.
**endurecimento**: pendente — sem acesso à imagem.

### Citação ABNT
(FORMSCHNEIDER), Unbekannt. **Pax und Respublica reichen sich die Hände, dahinter Caritas, Justitia und Prudentia.**. Herzog August Library. Disponível em: http://diglib.hab.de?grafik=35-5-geom-2f-2s-00004. Acesso em: 25 Jul. 2026.

### Proveniência
Candidato gerado automaticamente por `hunt.py` via europeana.
Score de relevância: **0.5** | Data: 2026-07-25

> **#verificar**: Esta nota requer validação visual e confirmação de escopo pela pesquisadora.
