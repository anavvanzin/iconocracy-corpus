---
id: SCOUT-560
tipo: corpus/candidato
status: candidato
titulo: Virtudes cardinais da Matriz de Nossa Senhora do Pilar, Ouro Preto
url: https://www.eba.ufmg.br/revistaceib/index.php/imagembrasileira/article/download/331/284/521
data_estimada: c. 1731–1750
pais: BR
regime: NORMATIVO
confianca: baixo
tags:
- corpus/candidato
- pais/BR
- regime/normativo
- null
related:
- '[[endurecimento]]'
- '[[Feminilidade de Estado]]'
data_scout: 2026-06-23
records_item_id: 8e44db21-f8d1-4024-a246-e0ac5ff24c9e
seculo: 18th
---

## Virtudes cardinais da Matriz de Nossa Senhora do Pilar, Ouro Preto

### Identificação
**Data**: c. 1731–1750 | **País**: BR | **Regime**: NORMATIVO
**URL**: [link](https://www.eba.ufmg.br/revistaceib/index.php/imagembrasileira/article/download/331/284/521)

**Motivos identificados**: figura feminina alegorica, dispositivo estatal brasileiro

### Análise preliminar de endurecimento
**Regime preliminar**: NORMATIVO

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
ACADÊMICO DO JULGAMENTO DE OURO PRETO. Identificação das alegorias das virtudes cardeais da Matriz de Nossa Senhora do Pilar. Imagem Brasileira, Belo Horizonte: EBA/UFMG.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `8e44db21-f8d1-4024-a246-e0ac5ff24c9e`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
