---
id: SCOUT-570
tipo: corpus/candidato
status: candidato
titulo: A América alegorizada em gravuras e frontispícios circulados no Brasil colonial
url: https://www.brasilianaiconografica.art.br/artigos/24232/a-representacao-da-america-quando-era-grande-e-desconhecida
data_estimada: séculos XVI–XVIII
pais: BR
regime: NORMATIVO
confianca: baixo
tags:
- corpus/candidato
- pais/BR
- regime/normativo
- null
related:
- '[[endurecimento]]'
- '[[Feminilidade de Estado]]'
data_scout: 2026-06-23
records_item_id: 6df4bc96-be48-4609-9974-18f48ad910ed
suporte: estampa
---

## A América alegorizada em gravuras e frontispícios circulados no Brasil colonial

### Identificação
**Data**: séculos XVI–XVIII | **País**: BR | **Regime**: NORMATIVO
**URL**: [link](https://www.brasilianaiconografica.art.br/artigos/24232/a-representacao-da-america-quando-era-grande-e-desconhecida)

**Motivos identificados**: figura feminina alegorica, dispositivo estatal brasileiro

### Análise preliminar de endurecimento
**Regime preliminar**: NORMATIVO

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
SOUZA, Ana Cecília Araújo Soares de. A América alegorizada: imagens e visões do Novo Mundo na iconografia europeia dos séculos XVI a XVIII. João Pessoa: Editora da UFPB, 2014.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `6df4bc96-be48-4609-9974-18f48ad910ed`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
