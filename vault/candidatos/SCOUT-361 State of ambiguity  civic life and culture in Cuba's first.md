---
id: SCOUT-361
tipo: corpus-candidato
status: candidato
titulo: "State of ambiguity : civic life and culture in Cuba's first republic"
acervo: "Library of Congress"
url: "https://www.loc.gov/item/2020719663/"
data_estimada: "2014-01-01"
pais: BR
suporte: estampa
motivo_alegorico: "Republic"
regime: INDETERMINADO
confianca: baixo
tags:
  - corpus/candidato
  - pais/BR
  - suporte/estampa
  - motivo/republica
  - #verificar
  - hunt-candidate
related:
  - "[[ENDURECIMENTO]]"
  - "[[Feminilidade de Estado]]"
hunt_score: 0.7
hunt_source: loc
data_scout: 2026-04-03
---

## State of ambiguity : civic life and culture in Cuba's first republic

### Identificação
**Criador/Gravador**: sanchez cobos, amparo
**Acervo**: Library of Congress
**URL**: [link](https://www.loc.gov/item/2020719663/)
**Data**: 2014-01-01 | **País**: BR | **Suporte**: estampa
**Thumbnail**: [imagem](https://tile.loc.gov/storage-services/master/gdc/gdcebookspublic/20/20/71/96/63/2020719663/2020719663.gif)

### Descrição
Includes bibliographical references (pages 323-348) and index. Description based on print version record; resource not viewed.

**Motivos identificados**: Republic

### Análise preliminar de ENDURECIMENTO
**Regime**: indeterminado — requer análise visual.
**ENDURECIMENTO**: pendente — sem acesso à imagem.

### Citação ABNT
AMPARO, sanchez cobos,. **State of ambiguity : civic life and culture in Cuba's first republic**. 2014-01-01. Library of Congress. Disponível em: https://www.loc.gov/item/2020719663/. Acesso em: 03 Apr. 2026.

### Proveniência
Candidato gerado automaticamente por `hunt.py` via loc.
Score de relevância: **0.7** | Data: 2026-04-03

> **#verificar**: Esta nota requer validação visual e confirmação de escopo pela pesquisadora.
