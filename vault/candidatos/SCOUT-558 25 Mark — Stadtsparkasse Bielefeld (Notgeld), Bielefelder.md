---
id: SCOUT-558
tipo: corpus/candidato
status: candidato
titulo: "25 Mark — Stadtsparkasse Bielefeld (Notgeld), Bielefelder Jungbronnen (Fountain of Youth), silk note"
url: "https://en.numista.com/210100"
data_estimada: "1921"
pais: DE
regime: FUNDACIONAL
confianca: baixo
tags:
  - corpus/candidato
  - pais/DE
  - regime/fundacional
  - #verificar
related:
  - "[[endurecimento]]"
  - "[[Feminilidade de Estado]]"
data_scout: 2026-06-23
records_item_id: 0b40bbaf-f789-5d6c-bce6-51d42b02b625
---
## 25 Mark — Stadtsparkasse Bielefeld (Notgeld), Bielefelder Jungbronnen (Fountain of Youth), silk note

### Identificação
**Data**: 1921 | **País**: DE | **Regime**: FUNDACIONAL
**URL**: [link](https://en.numista.com/210100)

**Motivos identificados**: Female allegory, Fountain of Youth, Healing, Municipal emergency currency

### Análise preliminar de endurecimento
**Regime preliminar**: FUNDACIONAL

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
STADTSPARKASSE BIELEFELD. 25 Mark (Notgeld). 1921. 1 cédula de seda (117 × 85 mm). Bielefeld, Alemanha. Catálogo Numista #210100. Disponível em: https://en.numista.com/210100.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `0b40bbaf-f789-5d6c-bce6-51d42b02b625`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
