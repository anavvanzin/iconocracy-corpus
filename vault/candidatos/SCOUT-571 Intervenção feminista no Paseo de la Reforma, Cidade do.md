---
id: SCOUT-571
tipo: corpus/candidato
status: candidato
titulo: Intervenção feminista no Paseo de la Reforma, Cidade do México, 2021
url: https://awarewomenartists.com/en/magazine/interventions-monumentales-feminisme-art-et-resistance-publique-au-mexique/
data_estimada: '2021'
pais: MX
regime: CONTRA-ALEGORIA
confianca: baixo
tags:
- corpus/candidato
- pais/MX
- regime/contra-alegoria
- null
related:
- '[[endurecimento]]'
- '[[Feminilidade de Estado]]'
data_scout: 2026-06-23
records_item_id: d55b1f29-20c6-4785-b7d5-814904b147d5
suporte: escultura
seculo: 21th
---

## Intervenção feminista no Paseo de la Reforma, Cidade do México, 2021

### Identificação
**Data**: 2021 | **País**: MX | **Regime**: CONTRA-ALEGORIA
**URL**: [link](https://awarewomenartists.com/en/magazine/interventions-monumentales-feminisme-art-et-resistance-publique-au-mexique/)

**Motivos identificados**: figura feminina alegorica, dispositivo estatal brasileiro

### Análise preliminar de endurecimento
**Regime preliminar**: CONTRA-ALEGORIA

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
AWARE. Monumental Interventions: Feminism, Art and Public Resistance in Mexico. 2021.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `d55b1f29-20c6-4785-b7d5-814904b147d5`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
