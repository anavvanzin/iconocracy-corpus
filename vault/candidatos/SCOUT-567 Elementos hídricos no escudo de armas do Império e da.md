---
id: SCOUT-567
tipo: corpus/candidato
status: candidato
titulo: "Elementos hídricos no escudo de armas do Império e da República"
url: "https://www.planalto.gov.br/ccivil_03/constituicao/brasao.htm"
data_estimada: "1822–1889–"
pais: BR
regime: NORMATIVO
confianca: baixo
tags:
  - corpus/candidato
  - pais/BR
  - regime/normativo
  - #verificar
related:
  - "[[endurecimento]]"
  - "[[Feminilidade de Estado]]"
data_scout: 2026-06-23
records_item_id: 68751372-c591-4853-b4b2-9ecd4873482e
---
## Elementos hídricos no escudo de armas do Império e da República

### Identificação
**Data**: 1822–1889– | **País**: BR | **Regime**: NORMATIVO
**URL**: [link](https://www.planalto.gov.br/ccivil_03/constituicao/brasao.htm)

**Motivos identificados**: figura feminina alegorica, dispositivo estatal brasileiro

### Análise preliminar de endurecimento
**Regime preliminar**: NORMATIVO

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
BRASIL. Lei que institui o brasão de armas do Império do Brasil. 18 de setembro de 1822.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `68751372-c591-4853-b4b2-9ecd4873482e`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
