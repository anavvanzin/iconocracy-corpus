---
id: SCOUT-565
tipo: corpus/candidato
status: candidato
titulo: "Personificação do Brasil em selos postais republicanos"
url: "https://iconocracy-corpus.local/piloto/"
data_estimada: "1889–1960"
pais: BR
regime: NORMATIVO
confianca: baixo
tags:
  - corpus/candidato
  - pais/BR
  - regime/normativo
  - #verificar
related:
  - "[[endurecimento]]"
  - "[[Feminilidade de Estado]]"
data_scout: 2026-06-23
records_item_id: a6a957f2-1181-400a-bc29-a8a17c3df07d
---
## Personificação do Brasil em selos postais republicanos

### Identificação
**Data**: 1889–1960 | **País**: BR | **Regime**: NORMATIVO
**URL**: [link](https://iconocracy-corpus.local/piloto/)

**Motivos identificados**: figura feminina alegorica, dispositivo estatal brasileiro

### Análise preliminar de endurecimento
**Regime preliminar**: NORMATIVO

*Análise gerada automaticamente a partir de records.jsonl. Requer validação visual.*

### Citação ABNT
CORREIOS. Selos postais da República dos Estados Unidos do Brasil com personificação feminina. 1889–1960.

### Proveniência
Nota gerada por `vault_sync.py push` em 2026-06-23.
Registro canônico: `data/processed/records.jsonl` item_id `a6a957f2-1181-400a-bc29-a8a17c3df07d`.

> **#verificar**: Esta nota requer validação visual e confirmação de escopo.
