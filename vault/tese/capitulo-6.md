...
> **Nota metodológica (junho/2026):** Os valores numéricos apresentados nesta seção refletem a primeira passagem de codificação (N=145, instrumento Gemma-4 via proxy textual). Uma segunda passagem com instrumento multimodal homogêneo (Fable-5, codificação direta da imagem) está em curso, com N=44 itens em escopo codificados até a data desta revisão. Os números abaixo serão atualizados mediante re-cálculo no notebook `notebooks/e1-pathosformel-analysis.ipynb` (célula §6.1-stats). As distribuições por regime e as correlações entre indicadores podem variar significativamente com a re-codificação, dado que a codificação visual direta eliminou o colapso em score zero (29/52 itens no piloto Gemma-4) causado por summaries textuais insuficientes.

O corpus analisado é composto por 145 artefatos visuais codificados integralmente, englobando suportes que transitam desde a grande estatuária oficial até a numismática e o meio impresso[^6.1-a]. O recorte geográfico reflete o circuito hegemônico de produção da cultura jurídica ocidental moderna, com predominância da França (42 itens) e dos Estados Unidos (20 itens), seguidos por Alemanha (18), Brasil (12), Reino Unido (10) e Itália (8)[^6.1-b].

[^6.1-a]: Ver `notebooks/e1-pathosformel-analysis.ipynb`, célula "corpus-overview", para a contagem atualizada por suporte. A população canônica de 265 registros em `data/processed/records.jsonl` inclui itens sem imagem acessível (216 marcados `#no-image`), que são excluídos da população analítica.

[^6.1-b]: A distribuição geográfica da re-codificação Fable-5 (N=44) difere da apresentada aqui por refletir apenas itens com imagem local resolvível. A distribuição completa será recalculada quando a campanha de reaquisição de imagens (branch `data/reacquire-images-*`) ampliar a população visual.

A métrica central desta análise é o *score* composto de endurecimento (variando teoricamente de 0 a 3), que agrega os dez indicadores ordinais de purificação corporal. No conjunto global do corpus, a média de endurecimento situa-se em 1.44 (com dispersão de 0.10 a 3.10)[^6.1-c]. Contudo, esse valor médio oculta variações cruciais quando os artefatos são agrupados por regime iconocrático.

[^6.1-c]: Na re-codificação Fable-5 (N=44 em escopo), a média preliminar é 1.59 (range 0.3–2.6), sem itens com score zero — o colapso em zero do Gemma-4 não se reproduziu na codificação visual direta. Ver `notebooks/e1-pathosformel-analysis.ipynb`, célula "fable5-descriptives".

O corpus distribui-se de maneira desigual entre os quatro regimes identificados: o regime *fundacional* é o mais populoso (71 itens), refletindo a proliferação de imagens no calor das rupturas constitucionais ou revolucionárias; segue-se o regime *normativo* (40 itens), característico da consolidação e rotinização do poder estatal; o regime *militar* ou imperial (27 itens); e, marginalmente, as *contra-alegorias* (7 itens), que funcionam como o reverso subversivo da iconografia oficial[^6.1-d].

[^6.1-d]: A distribuição preliminar da re-codificação Fable-5 (N=44) é: normativo 21, fundacional 13, militar 8, contra-alegoria 2. A inversão da dominância fundacional→normativa reflete a super-representação de numismática (moedas republicanas estabilizadas) no subconjunto com imagem local. A população completa pode restaurar a dominância fundacional observada no corpus Gemma-4.
