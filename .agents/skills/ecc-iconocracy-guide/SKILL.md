...
1. **Skills sob medida da tese** — vivem em `.Codex/skills/` (projeto). São a primeira escolha.
2. **Plugin ECC** (`ecc` v2.0.0-rc.1) — comandos de engenharia em `.Codex/commands/` e catálogo de skills em `.Codex/skills/ecc/`.
3. **Ferramentas MCP** — scite (literatura), Context7 (docs de bibliotecas), Exa (busca web).

> O agente também despacha por **palavras-gatilho** (modo routing do `AGENTS.md`), que muitas vezes
...
## 2. Modos do agente (AGENTS.md mode routing)
...
- O catálogo completo `.Codex/skills/ecc/` (`deep-research`, `article-writing`, `eval-harness`,
