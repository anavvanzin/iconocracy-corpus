...
2. **Placeholder-URL drift between ledger and export** — `records.jsonl` now carries **6** placeholder `input_url`s (`https://iconocracy.corpus/placeholder/…` / `iconocracy-corpus.local/piloto/`), down from 8, but `corpus/corpus-data.json` still surfaces **8** placeholder-bearing items. Two resolved in the ledger have not propagated to the export — re-run `records_to_corpus.py` and verify the 6 remaining against `data/raw/drive-manifest.json`.
   - Minor dedup watch: **4 duplicate real `input_url`s** in `records.jsonl` (3× `iconocracy-corpus.local/piloto/` pilot rows + 3 real-source pairs) — candidates for `corpus-dedup`, not confirmed duplicates.
