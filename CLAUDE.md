...
3. **Analysis pinned at N=165 while corpus grew to 265** — the quantitative analysis (notebooks + manuscript) was run against an older 165-item snapshot:
   ...
   - **DECISION PENDING (Ana) — reframed 2026-05-30, see `docs/decisions/DIALETICA-N165-vs-265.md`:** the "165 vs 265" framing was rejected by adversarial review. Real axis = **coding-validity stratum × instrument provenance**, not date. The corpus is coded by 6 instruments (`coded_by`); **41 items are uncoded** (no `coded_by`/regime/endurecimento) → N=265 is NOT a valid analytic N. Verified options (`audit-coded-by-2026-06-10.md`): **N=145** (IconoCode-only: opus 100 + opus-4.6 45) or **N≈165** (+19 `migration` coins AFTER re-coding via IconoCode; +FR-048 manual). **N≈223 is NOT defensible** — `vault-import` (58) is all-zero placeholder (0 variance) and `migration` (19) has null scores; both verified inert/uncoded, not genuine coding. Inter-instrument protocol (opus vs opus-4.6): `IRR-INTER-INSTRUMENTO-2026-06-10.md`. Action: stratify by `coded_by` → quarantine the 41 uncoded → reliability audit → set analytic N by validity → dataset card in Cap2 replacing "N=165" with "N=[valid]". **CORRECTION (verified 2026-06-10):** `endurecimento_score=0` is NOT a valid score in the current data — 0/145 IconoCode items score 0; all 99 zeros are placeholders (41 uncoded + 58 vault-import all-zero). Treat 0 as "not measured", not "low purification". Notebooks read `corpus_dataset.csv` (165 rows, currently 0 all-zero — clean); regenerating it from the 264-corpus via `code_purification.py --export-csv` would import the 58 vault-import zeros — filter by `coded_by`/validity before export.
