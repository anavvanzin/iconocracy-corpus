...
def main():
    ...
    # 0. Excluir Duplicados Reais (Exorcismo Imediato)
    IDS_A_DELETAR = {"82c0efc9-1a2e-55a3-8eca-5de3e58a2fac", "68a00893-7b89-5e58-8aef-79f89b547974"}
    
    records_originais = load_jsonl(RECORDS_JSONL)
    records_limpos = [r for r in records_originais if r.get("id") not in IDS_A_DELETAR]
    
    purification_originais = load_jsonl(PURIFICATION_JSONL)
    purification_limpos = [p for p in purification_originais if p.get("id") not in IDS_A_DELETAR]
    
    if len(records_limpos) < len(records_originais):
        write_jsonl(RECORDS_JSONL, records_limpos)
        write_jsonl(PURIFICATION_JSONL, purification_limpos)
        print(f"Exorcizados {len(records_originais) - len(records_limpos)} duplicados reais!")
